...
def summarize_old_history(chat_history, model_name, host_url, max_tokens=512):
    ...
    to_summarize = ""
    for sender, message, timestamp in chat_history[:-5]:
...
with st.sidebar.form("settings_form"):
    ...
    selected_model = st.session_state.get('model', "llama3")  # st.selectbox("🤖 Model", models, key="selected_model")
    temperature = st.session_state.get('temperature', 0.8)  # st.slider("🌡️ Teplota", 0.0, 1.0, 0.8, 0.01, key="temperature")
    max_tokens = st.session_state.get('max_tokens', 512)  # st.slider("🔢 Max. tokenů v odpovědi", 50, 4096, 512, 50, key="max_tokens")
    top_p = st.session_state.get('top_p', 0.95)  # st.slider("📊 Top-p", 0.0, 1.0, 0.95, 0.01, key="top_p")
    gpu_layers = st.session_state.get('gpu_layers', 20)  # st.slider("🧠 GPU layers", 0, 80, 20, 1, key="gpu_layers")
    use_memory = st.session_state.get('use_memory', False)  # st.checkbox("🧠 Zapnout paměť", value=False, key="use_memory")

    apply_settings = st.form_submit_button("✅ Použít nastavení")
...
if apply_settings:
    ...
    if not available_models:
        ...
        if st.session_state.selected_model in available_models:
            ...
            selected_model_index = 0
            st.session_state.selected_model = st.session_state.get('model', "llama3")  # available_models[0]
    st.session_state.selected_model = st.session_state.get('model', "llama3")  # st.selectbox(
        "🧠 Vyber model",
        options=available_models,
        index=selected_model_index,
        help="Modely dostupné na vaší Ollama instanci. Formát: název:digest"
    )
    temperature = st.session_state.get('temperature', 0.8)  # st.slider("🌡️ Teplota (kreativita)", 0.0, 2.0, 0.7, 0.1,
                            help="Nižší hodnota = více deterministické, vyšší = kreativnější.")
    max_tokens = st.session_state.get('max_tokens', 512)  # st.slider("🔢 Max. tokenů v odpovědi", 50, 4096, 512, 50,
...
with st.sidebar.expander("🧠 Aktuální paměť"):
    st.markdown("### 🧠 Shrnutí:")
    st.markdown(st.session_state.get("memory_summary", "_(Shrnutí není dostupné)_"))

    if "recent_memory" in st.session_state:
        st.markdown("---")
        st.markdown("### 🔁 Krátkodobá paměť:")
        for msg in st.session_state.recent_memory:
    ...
    if not model_name_for_api or "Nebyly nalezeny modely" in model_name_for_api:
        ...
        payload = {
            ...
            "options": {
                ...
                "num_predict": max_tokens,
                "num_gpu_layers": 20  # ← zde nastavíš počet vrstev do GPU (zbytek CPU)
            },
            "stream": stream_mode
        }
        try:
