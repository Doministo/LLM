...
with st.sidebar:
    ...
    # 🔄 Možnost ručního reloadu modelů
    if st.button("🔄 Načíst modely znovu"):
    ...
    with st.form("settings_form"):
        ...
        # 📦 Načtení modelů z cache nebo API
        ...
        default_model = st.session_state.get("selected_model", "Zvolte model")
        if default_model not in models_display:
            default_model = "Zvolte model"

        selected_model = st.selectbox(
            "🤖 Vyber model",
            models_display,
            index=models_display.index(default_model),
            key="selected_model"
        )


        temperature = st.slider("🌡️ Teplota", 0.0, 1.0, 0.8, 0.01, key="temperature")
        max_tokens = st.slider("🔢 Max. tokenů v odpovědi", 50, 4096, 512, 50, key="max_tokens")
        top_p = st.slider("📊 Top-p", 0.0, 1.0, 0.95, 0.01, key="top_p")
        gpu_layers = st.slider("🧠 GPU layers", 0, 80, 20, 1, key="gpu_layers")
        use_memory = st.checkbox("🧠 Zapnout paměť", value=False, key="use_memory")

        cz_mode = st.checkbox("🇨🇿 Vynutit češtinu", value=False, key="cz_mode")
        stream_mode = st.checkbox("🌀 Streamování odpovědí", value=True, key="stream_mode")
        show_tokens = st.checkbox("🔢 Zobrazovat tokeny", value=True, key="show_tokens")
    ...
    with st.expander("🧠 Aktuální paměť"):
        st.markdown("### 🧠 Shrnutí:")
