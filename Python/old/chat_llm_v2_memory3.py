...
with st.sidebar:
    ...
    if st.button("❌ Vymazat historii chatu"):
        st.session_state.chat_history = []
        st.rerun()
    gpu_layers = st.slider("🧠 Počet vrstev na GPU", 0, 80, 20, 1,
                       help="Nižší = méně VRAM, ale pomalejší inference.")
    ...
    if not model_name_for_api or "Nebyly nalezeny modely" in model_name_for_api:
        ...
        payload = {
            ...
            "options": {
                ...
                "num_predict": max_tokens,
                "num_gpu_layers": gpu_layers  # ← zde nastavíš počet vrstev do GPU (zbytek CPU)
            },
            "stream": stream_mode
        }
        try:
